...
var _tersify = require('./tersify');

var _tersify2 = _interopRequireDefault(_tersify);

var _versions = require('./tersify/versions');
...
var TerserPlugin = function () {
  function TerserPlugin() {
    ...
    var _options$terserOption = options.terserOptions,
        terserOptions = _options$terserOption === undefined ? {} : _options$terserOption,
  ...
  _createClass(TerserPlugin, [{
    ...
    value: function apply(compiler) {
      ...
      var optimizeFn = function optimizeFn(compilation, chunks, callback) {
        var tersify = new _tersify2.default({
          ...
          try {
            ...
            if (_this.options.cache) {
              task.cacheKey = (0, _serializeJavascript2.default)({
                terser: _versions2.default.terser,
                'terser-webpack-plugin': _versions2.default.plugin,
            ...
            compilation.errors.push(TerserPlugin.buildError(error, file, TerserPlugin.buildSourceMap(inputSourceMap), requestShortener));
          }
        });

        tersify.runTasks(tasks, function (tasksError, results) {
          if (tasksError) {
          ...
          results.forEach(function (data, index) {
            ...
            if (error) {
              compilation.errors.push(TerserPlugin.buildError(error, file, sourceMap, requestShortener));
            ...
            if (warnings && warnings.length > 0) {
              warnings.forEach(function (warning) {
                var builtWarning = TerserPlugin.buildWarning(warning, file, sourceMap, _this.options.warningsFilter, requestShortener);
